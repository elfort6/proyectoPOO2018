<div class="row">
    <div class="col-12 col-md-3">
        <div class="form-group">
            <label for="sel2" class="encabezado-tabla my-2">Sección:</label>
            <select class="form-control" id="sel2">
                <option value=""></option>
                <option>0700</option>
                <option>0800</option>
                <option>0900</option>
                <option>1000</option>
                <option>1100</option>
            </select>
        </div>
    </div>
    <div class="col-12 col-md-3">
        <div class="form-group">
            <label for="sel1" class="encabezado-tabla my-2">Asignatura:</label>
            <select class="form-control seleccion-asignatura" id="sel1">
                <option value=""></option>
            </select>
        </div>
    </div>
    <div class="col-12 col-md-3">
        <div class="form-group">
            <label for="sel3" class="encabezado-tabla my-2">Docente:</label>
            <select class="form-control seleccion-docente">
                <option value=""></option>
            </select>
        </div>
    </div>
    <div class="col-12 col-md-3">
        <div class="form-group">
            <label for="sel4" class="encabezado-tabla my-2">Cupos:</label>
            <input type="number" class="form-control" id="sel4">
        </div>
    </div>

    <div class="col-12 col-md-3">
        <div class="form-group">
            <label for="sel5" class="encabezado-tabla my-2">Dias:</label>
            <input type="text" class="form-control" id="sel5" placeholder="LuMaMiJuViSa">
        </div>
    </div>
    <div class="col-12 col-md-3">
        <div class="form-group">
            <label for="sel6" class="encabezado-tabla my-2">Periodo:</label>
            <select class="form-control" id="sel6">
                <option value=""></option>
                <option>1</option>
                <option>2</option>
                <option>3</option>
            </select>
        </div>
    </div>

    <div class="col-12 col-md-3">
        <div class="form-group">
            <label for="sel7" class="encabezado-tabla my-2">Hora Inicial:</label>
            <select class="form-control" id="sel7">
                <option value=""></option>
                <option>0700</option>
                <option>0800</option>
                <option>0900</option>
                <option>1000</option>
                <option>1100</option>
            </select>
        </div>
    </div>
    <div class="col-12 col-md-3">
        <div class="form-group">
            <label for="sel8" class="encabezado-tabla my-2">Hora Final:</label>
            <select class="form-control" id="sel8">
                <option value=""></option>
                <option>0700</option>
                <option>0800</option>
                <option>0900</option>
                <option>1000</option>
                <option>1100</option>
            </select>
        </div>
    </div>
    <div class="col-12 col-md-3">
        <div class="form-group">
            <label for="sel9" class="encabezado-tabla my-2">Edificio:</label>
            <select class="form-control" id="sel9">
                <option value=""></option>
                <option>F1</option>
                <option>D1</option>
                <option>B2</option>
                <option>E1</option>
            </select>
        </div>
    </div>
    <div class="col-12 col-md-3">
        <div class="form-group">
            <label for="sel10" class="encabezado-tabla my-2">Aula:</label>
            <input type="number" class="form-control" id="sel10">
        </div>
    </div>
</div>