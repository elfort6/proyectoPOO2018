<div class="container forma03-encabezado">

    <div class="card mb-3 shadow-sm">
        <div class="card-header">
            <h4 class="my-0 font-weight-normal text-center">MM-110</h4>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-6">
                    <h3 class="card-title pricing-card-title mx-5">POO</h3>
                    <ul class="list-unstyled mt-3 mb-4 mx-5">
                        <li>Sección: 1100</li>
                        <li>Hora inicial: 11:00</li>
                        <li>Hora final: 12:00</li>
                        <li>Edificio: f1</li>
                        <li>Aula:110</li>
                        <li>Días: LuMaMiJuVi</li>
                        <li>Alumnos matriculados: 45</li>
                    </ul>
                </div>
                <div class="col-6">
                    <h3 class="card-title pricing-card-title mx-5">Descargar Listado de Alumnos:</h3>
                    <div class="mt-3 mb-4 mx-5">
                        <span style="font-size: 3em; color: red;">
                            <i class="far fa-file-pdf icono icono1" title="PDF"></i>
                        </span>
                        <span style="font-size: 3em; color: green;">
                            <i class="far fa-file-excel icono icono2" title="EXCEL"></i>
                        </span>
                    </div>
                </div>
            </div>
        </div>
    </div>

</div>